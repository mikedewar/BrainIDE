...
\DeclareMathOperator{\bpsi}{\boldsymbol \psi}
\begin{document}
%
% paper title
...
\author{Parham Aram*,
        Dean~R.~Freestone,~\IEEEmembership{Graduate Student Member,~IEEE,} Kenneth Scerri, Michael Dewar and Visakan Kadirkamanathan,~\IEEEmembership{Member,~IEEE}}  % <-this % stops a space
...
\IEEEPARstart{N}{on-linear} spatio-temporal behaviour...  %WEINAND1972
...
The dynamics of the $p$th order non-linear, stochastic, spatio-temporal IDE
 is described by
 \begin{equation}
  \epsilon_t(\mathbf r)=\int_{\Omega}w(\mathbf r,\mathbf r')f(v_{t-p}(\mathbf r'))d\mathbf r'+\mathbf e_{t-p}(\mathbf r)
 \end{equation}
where $\epsilon_t(\mathbf r)$ is a lag polynomial defined as
\begin{equation}
 \epsilon_t(\mathbf r)=(1+\sum_{i=1}^{p}\varphi_i L_i)v_{t}\left(\br\right)
\end{equation}

where $v_t(\mathbf r)$ denotes the spatial field at time $t$ and spatial location $\mathbf r$, $f(.)$ is some nonlinear function, and $w(.)$ is the spatial kernel. Here we consider the case where $p=1$, therefore we have
\begin{equation}
v_t(\mathbf r) +\varphi_{1} v_{t-1}(\mathbf r)=\int_{\Omega}w(\mathbf r,\mathbf r')f(v_{t-1}(\mathbf r'))d\mathbf r'+\mathbf e_{t-1}(\mathbf r)
\end{equation}
...
% \bibliography{IEEEabrv,../../../Papers/SpatioTemporalSystems,../../../Papers/Wavelet}
