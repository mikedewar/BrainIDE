...
Estimating function cortical connectivity via EEG measures is currently a highly active area of research. There is a lot of interest in understanding the hierarchy of brain regions are involved in specific tasks. Most works are based on auto-regressive (AR) modeling of the times series. Studies have concentrated on finding functional connectivity patterns using information contained in the AR coefficients(Granger causality, DTF, partial coherence). An alternative approach to estimating the connectivity structure between brain regions is dynamic causal modeling (DCM). The DCM approach essentially studies the causality of information transform between pre-determined brain regions. These regions are determined from prior information from fMRI, AR modeling or source inversion for example. DCM asks what connectivity structure between the specific isolated brain regions best explains a data set. The approach has generated several publications (ref) and generated evidence in understanding specific hierarchical networks of neural information processing. These approaches have yielded interesting results but is limited in the sense that structure is not estimated hence this work cannot be related to the theoretical bifurcation analysis and the underlying system structure.
...
In this section we describe a variant of the Jansen and Ritt neural field model~\cite{Jansen1995} that will be used to create our connectivity estimator. This model has shown to be descriptive of a range of neurodynamics of the cortex such as evoked potentials and epileptic behavior and is capable of generating complex patterns of activity such as Turing patterns, spirals and traveling oscillations. The general form of the model is described here. The post-synaptic potentials generated at a neuronal population $s$ by action potentials arriving from all other connected populations at position $r$ can be described by
