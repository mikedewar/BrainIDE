...
% 	w\left(r,r'\right) =&& \\
% &&\mathbf{\theta}_1\exp{\left(-\frac{(r-r')^\top(r-r')}{\sigma_1^2}\right)}+\mathbf{\theta}_2\exp{\left(-\frac{(r-r')^\top(r-r')}{\sigma_2^2}\right)}+\mathbf{\theta}_3\exp{\left(-\frac{(r-r')^\top(r-r')}{\sigma_3^2}\right)}\nonumber
% \end{eqnarray}
% where $\sigma_i$ is the scale parameter.
% % where
% % \begin{equation}
% % 	\mathbf{\theta}=\begin{bmatrix} \mathbf{\theta}_{1}\\\mathbf{\theta}_{2}\\\mathbf{\theta}_{3}\end{bmatrix},\quad 		\Sigma_w=\begin{bmatrix}\Sigma_{w1}\\\Sigma_{w2}\\ \Sigma_{w3}\end{bmatrix}.
% % \end{equation}
% \begin{todo}
