...
\end{eqnarray}
We can not calculate the posterior in this Bayesian model for some reason (Mike and Ken can ellaborate), so we need to decompose the field into weighted basis functions. Therefore we define
...
\end{equation}
Will will assume we know the parametric form of the basis functions, where the parameter $\theta$ is unknown. Each connectivity basis function can be considered a layer in the WC model. Making substitutions we get
