...
The average firing rate of the presynaptic neurons rate to the pre-synaptic membrane potential by the Heaviside step function
\begin{equation}\label{Eq4}
P\left( {v\left( {r,\bar t} \right)} \right) = {r_0}S\left( {v\left( {r,\bar t} \right) - {v_0}\left( r \right)} \right)
\end{equation}
\begin{equation}\label{Eq4.1}
S\left( {v\left( {r,\bar t} \right) - {v_0}\left( r \right)} \right) = \left\{ {\begin{array}{*{20}{c}}
   1 & {v\left( {r,\bar t} \right) \ge {v_0}\left( r \right)}  \\
   0 & {otherwise}  \\
\end{array}} \right.
\end{equation}
Even though $v_0$ is a constant in this research we show express it as a field allowing for inhomogeneities and for clarity and generality in constructing the state-space representation in the next section. The parameters in equation~\ref{Eq4} are... $v_0$ is typically 6~mV, $r_0$ is 5~$s^{-1}$ (from Wendling and David papers). By combining equations~\ref{Eq1} and \ref{Eq3} we get the spatiotemporal model
\begin{equation}\label{Eq5}
v\left(s,t\right) = \int\limits_{-\infty}^t\int\limits_\Omega  h\left(t - t'\right)W\left(s,r\right)P\left( v\left( r,\bar t \right)\right)dr dt'+E\left(s,t'\right)
\end{equation}
...
An alternate and equivalent way to remove the temporal integration is through the Laplace transform. We start by considering a discrete neural mass $i$ with inputs with zero delay from another discrete mass $j$ and
\begin{equation}\label{Eq9.1}
{v_i}\left( t \right) = \int_{ - \infty }^t {{h_j}(t - t'){w_{ij}}P\left( {{v_j}\left( t \right)} \right)dt'},
\end{equation}
where $w_{ij}$ can be considered the synaptic efficiency from $j$ to $i$. Now we take the Laplace transform yielding
\begin{eqnarray}
% \nonumber to remove numbering (before each equation)
  {V_i}\left( s \right) &=& {H_j}\left( s \right){w_{ij}}P\left( {{V_j}\left( s \right)} \right) \nonumber \\
   &=& {\left( {s + \frac{1}{\tau }} \right)^{ - 1}}{w_{ij}}P\left( {{V_j}\left( s \right)} \right)  \\
  \Rightarrow s{V_i}\left( s \right) + \frac{1}{\tau }{V_i}\left( s \right) &=& {w_{ij}}P\left( {{V_j}\left( s \right)} \right).
\end{eqnarray}
Now taking the inverse Laplace transform we get
\begin{equation}\label{9.3}
\frac{{d{v_i}\left( t \right)}}{{dt}} + \frac{1}{\tau }{v_i}\left( t \right) = {w_{ij}}P\left( {{v_j}\left( t \right)} \right).
\end{equation}
Next we convert the model from discrete neural masses to a continuum by
\begin{equation}\label{Eq9.4}
\frac{{dv\left( {s,t} \right)}}{{dt}} + \frac{1}{\tau }{v\left( {s,t} \right)} = \int_\Omega  {W\left( {s,r} \right)P\left( {v\left( {r,\bar t} \right)} \right)dr}  + E\left( {s,t} \right).
\end{equation}
