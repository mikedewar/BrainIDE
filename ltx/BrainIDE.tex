...
Until now, estimation of local connectivity structure has not been attempted. Recently it has been shown that it is possible to estimate local connectivity of systems governed by integro-difference equations by formulating a state-space model~\cite{Dewar2009}. In addition, the appropriate model selection tools have been developed~\cite{Scerri2009} allowing for the application of this technique to neural fields. In this paper we demonstrate how intracortical connectivity can be inferred from EEG data, based on a variant Jansen and Ritt neural field model.
