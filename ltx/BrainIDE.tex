...
\begin{equation}\label{DefKernelDecomp}
	w\left(\mathbf{r}-\mathbf{r}'\right) = \boldsymbol{\theta}^{\top} \mathbf{\psi}\left(\mathbf{r}-\mathbf{r}'\right)
\end{equation}
...
\begin{equation}\label{reduced continuous model}
	\boldsymbol{\phi}^{\top}\left( \mathbf{r} \right) \mathbf{x}_{t+1} = T\boldsymbol{\theta}^{\top}\int_\Omega  {\psi \left(\mathbf{r}-\mathbf{r}'\right) f\left( \boldsymbol{\phi} ^{\top}\left(\mathbf{r}'\right)\mathbf{x}_t \right)d\mathbf{r}'}  - \lambda\boldsymbol{\phi}^{\top}\left(\mathbf{r}\right)x_t + T u_t\left(\mathbf{r}\right) + e_t\left(\mathbf{r}\right).
\end{equation}
...
\begin{eqnarray}\label{Homogeneous SS Model}
	...
	q(\mathbf{x}_t,\boldsymbol{\theta},\alpha) &=& T\boldsymbol{\Gamma}^{ - 1}\int_\Omega {\boldsymbol{\phi}\left(\mathbf{r}\right) \int_\Omega {f\left(\boldsymbol{\phi}^{\top}\left(\mathbf{r}'\right)\mathbf{x}_t\right)\psi^{\top} \left(\mathbf{r}-\mathbf{r}'\right)d\mathbf{r}'} d\mathbf{r}} \boldsymbol{\theta} - \lambda \mathbf{x}_t \\
...
\begin{equation}\label{Poo}
	\int_\Omega  {\boldsymbol{\phi} \left(r\right)\boldsymbol{\phi}^{\top}\left(r\right) dr} x_{t+1} = T \int_\Omega  {\boldsymbol{\phi} (r) \boldsymbol{\theta}^{\top} \int_\Omega  {\psi \left(|r-r'|\right)f\left(\boldsymbol{\phi}^{\top}\left(r'\right) x_t \right)dr'}dr}  - \lambda\int_\Omega {\boldsymbol{\phi}(r)\boldsymbol{\phi}^{\top}(r)dr} x_t + T \int_\Omega{\boldsymbol{\phi} \left(r\right) I_t\left(r\right)dr} + \int_\Omega{\boldsymbol{\phi} \left(r\right) e_t\left(r\right)dr}.
\end{equation}
...
\begin{equation}\label{Homogeneous SS Model}
	x_{t+1} = T\Gamma^{ - 1}\int_\Omega {\boldsymbol{\phi}\left(r\right) \int_\Omega {f\left(\boldsymbol{\phi}^{\top}\left(r'\right)x_t\right)\psi^{\top} \left(|r-r'|\right)dr'} dr} \boldsymbol{\theta} - \lambda x_t + \Gamma^{-1}T \int_\Omega{\boldsymbol{\phi} \left(r\right) u_t\left(r\right)dr} + \Gamma^{-1}\int_\Omega{\boldsymbol{\phi}\left(r\right)e_t\left(r\right)dr}.
\end{equation}
...
\begin{equation}
x_{t + 1} = T q(x_t)\boldsymbol{\theta} -\lambda x_t + \Gamma^{-1}\int_\Omega  {\boldsymbol{\phi} ( \mathbf{r} )e_t( \mathbf{r} )d\mathbf{r}}
\end{equation}
