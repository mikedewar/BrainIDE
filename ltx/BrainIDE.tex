...
\author{Dean~R.~Freestone,~\IEEEmembership{Graduate Student Member,~IEEE,}
        ...
        Visakan~Kadirkamanathan,~\IEEEmembership{Member,~IEEE,}% <-this % stops a space% <-this % stops a space
...
\begin{equation}\label{eq:FieldCovariance}
...
\begin{equation} 
\boldsymbol{\Sigma}_e=\boldsymbol{\Gamma}^{-1}\iint\limits_{\Omega}\boldsymbol{\phi}\left(\mathbf r\right) \rho\left(\mathbf{r}- \mathbf{r}' \right)\boldsymbol{\phi}\left(\mathbf{r}'\right)^{\top}d\mathbf{r}' d\mathbf{r}\boldsymbol{\Gamma}^{- \top} 
\end{equation}
...
There are two options available to alleviate problems with aliasing. The first is to increase the spatial sampling frequency (using a higher resolution of sensors), and the second is to use wider sensors, which act like an anti-aliasing filters. The first option is more desirable, since more information is retained. If sensors are modelled as having a Gaussian pickup range than the slope of the attenuation will be gentle allowing for some low amplitude high frequency components to still be present and mild aliasing will still occur. In practice, the bandwidth of the cortex can not be estimated by traditional electrophysiological measurements due to spatial limitations. It may be estimated using other modalities with higher spatial resolution such as, fMRI, NIRS or other optical imaging techniques. Errors associated with mild aliasing result in distortions in the high frequency components of the field dynamics. However, after applying the basis function decomposition, high frequency component contribute negligibly to the inverse problem and are therefore not a major problem~\cite{Sanner1992}. This is further illustrated our results.

Similar considerations need to be made regarding the representation of the dynamic field $w_t(\mathbf{r})$ by its basis functions decompositions. The minimum distance between adjacent basis functions needs to again satisfy Shannon's sampling theorem. Thus the minimum distance between basis must again satisfy $\frac{1}{2\nu_c}$. For the Gaussian basis functions $\phi(\mathbf{r})$ being considered, the basis width parameter $\sigma^2$ can also be inferred from spectral considerations \cite{Sanner1992,Scerri2009}. The Fourier transform of a Gaussian is another Gaussian with width $\sigma^2_{\nu}$ given by $\sigma^2_{\nu} = \frac{1}{\pi^2\sigma^2}$. Thus to ensure that the basis functions can represent fields with frequency content up to $\boldsymbol{\nu}_c$, $\sigma_{\nu}$ should be set with the 3dB frequency for the basis functions equal to $\boldsymbol{\nu}_c$, thus obtained $\sigma^2_{\nu}$ and therefore $\sigma^2$. 
...
\State Initialise
\begin{equation*}
\hat{\mathbf x}_0=\mathbb E[\mathbf x_0]
\end{equation*}
\begin{equation*}
\mathbf P_0=\mathbb E[(\mathbf x_0-\hat{\mathbf x}_0)(\mathbf x_0-\hat{\mathbf x}_0)^\top]
\end{equation*}
\State Form the sigma vectors 
\begin{equation*}
\mathcal X_k=[\hat{\mathbf x}_k \quad \hat{\mathbf x}_k+\gamma\sqrt{\mathbf P_k} \quad \hat{\mathbf x}_k-\gamma\sqrt{\mathbf P_k}]
\end{equation*}
\State Update the sigma vectors by propogating them through state transition equation and compute the predicted state and the predicted state covariance
\begin{equation*}
 \hat{\mathbf x}_{k +1|k}=\sum_{i=0}^{2L} W_i^{(m)}\mathcal X_{i,k+1|k}
\end{equation*}
\begin{equation*}
 \mathbf P_{k +1|k}=\sum_{i=0}^{2L} W_i^{(c)}(\mathcal X_{i,k+1|k}-\hat{\mathbf x}_{k +1|k})(\mathcal X_{i,k+1|k}-\hat{\mathbf x}_{k +1|k})^\top+\boldsymbol \Sigma_w
\end{equation*}
...
\State use maxima to form the spline $r_k(n) = s_{max,k}(n)$
...
\begin{equation}
\boldsymbol\Sigma_e =\mathbf{\Gamma}^{-1}\iint\limits_{\Omega}\boldsymbol{\phi}\left(\mathbf r\right) \beta\left(\mathbf r- \mathbf r' \right)\boldsymbol{\phi}\left(\mathbf r'\right)^{\top}d\mathbf r' d\mathbf r\mathbf{\Gamma}^{- \top} 
\end{equation}
...
&=&\mathbf{\Gamma}^{-1}\iint\limits_{\Omega}\boldsymbol{\phi}\left(\mathbf r\right) \gamma\left(\mathbf r- \mathbf r' \right)\boldsymbol{\phi}\left(\mathbf r'\right)^{\top}d\mathbf r' d\mathbf r\mathbf{\Gamma}^{- \top} 
\end{eqnarray}
\end{IEEEproof}
