...
\end{align}
\dean{The solution of this equation for $\mathbf{x}_t$ is equivalent to the Galerkin projection. To show this we define the residual
\begin{align}
...
\end{equation}
where $n_x$ is the number of states and basis functions. If equation~\ref{eq:GalerkinProjection} is satisfied then the basis functions are orthogonal to the residual. If one was using the Galerkin projection to estimate the states, they would numerically solve the $n_x$ equations to estimate the state vector, which is a standard method in economics modeling. The method in this paper takes a different approach by forming the state-space model. To show how this is achieved we substitute the field decomposition into the right hand side of equation~\ref{eq:decompRHS} giving
\begin{align}
