...
For models to be clinically viable, they must be patient-specific. A possible approach to achieve this would be to fit a general continuum neural field model, like the \citet{Wilson1973} (WC) or \citet{Amari1977} models, or a neural mass model like the \citet{Jansen1995} model, to patient-specific data. Fitting the neural models to individuals is a highly non-trivial task. Recently, however, this task has been approached from a number of standpoints. \dean{(Reviewer 1, additional comment d):) The first paper on patient specific modeling came from~\citet{Valdes1999}, where they fit a neural mass model of \citet{LopesDaSilva1976} and \citet{Zetterberg1978} \parham{using an extended Kalman filter--need to check (it's not extended Kalman filter I think-maybe just say using nonlinear Kalman filtering algorithm or maybe using Local Linearisation filter \citep{Ozaki1994}}. This paper demonstrated for the first time the feasibility of assimilating data with neural models of EEG.} 
...
\end{align}
\dean{The solution of this equation for $\mathbf{x}_t$ is equivalent to finding a Galerkin projection, where the field basis functions would be considered the Galerkin projection functions and the states would be the amplitude coefficients. If the Galerkin approach was used estimate the states, $n_x$ coupled ODEs would be solved independently to estimate the state vector, which is a standard method in fluid dynamics and economics modeling. The method in this paper takes a different approach by forming the state-space model, allowing for more efficient estimation. To show how this is achieved we substitute the field decomposition into the left hand side of equation~\ref{eq:decompRHS} giving
\begin{align}
...
\end{equation}
\parham{(reviewer 2, comment 7:) The positive constant $\alpha$, that determines the spread of the sigma points around $\mathbf{\bar x}$, is typically set arbitrary small (e.g., $10^{-3}$) to minimise higher order effect \citep{Haykin2001}, and  $\kappa$ is an additional scaling parameter which was set to $3-n_x$ in accordance to \citet{Julier2002a}.}
...
\section{Equivalence to Galerkin's Projection}
To show how to solution of equation for the state vector is equivalent to the Galerkin projection we define the residual between the field and the decomposition as 
\begin{align}
	\Xi(\mathbf{x}_t,v_{t+1}(\mathbf{r})) &= v_{t+1}(\mathbf{r}) - T_s\int_\Omega{f(\boldsymbol{\phi}^{\top}(\mathbf{r}')\mathbf{x}_t )\boldsymbol{\psi}^{\top}(\mathbf{r}-\mathbf{r}') \, d\mathbf{r}'}\boldsymbol{\theta} \nonumber \\
	&+ \xi\boldsymbol{\phi}^{\top}(\mathbf{r})\mathbf{x}_t + \mathbf{e}_t(\mathbf{r}).
\end{align} 
The Galerkin projection method finds $\mathbf{x}_t$, such that for all of the basis functions, the inner product with the residual satisfies
\begin{equation}\label{eq:GalerkinProjection}
	G_i = \int_{\Omega}\Xi(\mathbf{x}_t,v_{t+1}(\mathbf{r})) \phi_i(\mathbf{r}) d\mathbf{r} = 0, \> i=0,1,\hdots,n_x-1,
\end{equation}
where $n_x$ is the number of states and basis functions. If the inner product between the residual and the basis functions in equation~\ref{eq:GalerkinProjection} is zero, then the basis functions are orthogonal to the residual. 
