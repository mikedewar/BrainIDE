...
\end{equation}
where $\boldsymbol{\psi}(\mathbf{r},\mathbf{r}')$ is a vector of Gaussian basis functions and $\boldsymbol{\theta}$ is a vector of scaling parameters. \dean{A graphical demonstration of the connectivity kernel decomposition is provided in Figure~\ref{fig:Figure1}.} By exploiting the isotropy of the connectivity basis functions, they can be written as  
...
% \begin{equation}
% 	R_1(\boldsymbol{\tau}) = \xi R_{\bar{y}_{t},\bar{y}_{t}}(\boldsymbol{\tau}),
% \end{equation}
% which is the spatial autocorrelation of the observations scaled by the decay parameter $\xi$.
% 
...
% \end{equation}
% The right hand side of equation~\ref{eq:support_estimator} will have the structure of the connectivity kernel. To show this let
% \begin{align}
% 	Z &= \mathcal{F}\left\{\bar{y}_t\left( \mathbf{r} + \boldsymbol{\tau} \right) \int_{\Omega}{ w\left(\mathbf{r},\mathbf{r}'\right) \bar{y}_t\left( \mathbf{r}' \right)\, d\mathbf{r}'}\right\} \\
% 	&= \exp(i\boldsymbol{\nu}^{\top}\boldsymbol{\tau})\bar{Y}(\boldsymbol{\nu}) \ast W(\boldsymbol{\nu})\bar{Y}(\boldsymbol{\nu}) \\
% 	&= \exp(i\boldsymbol{\nu}^{\top}\boldsymbol{\tau})W(\boldsymbol{\nu})\bar{Y}(\boldsymbol{\nu}) \ast \bar{Y}(\boldsymbol{\nu}),
% \end{align}
% using the associativity with scalar multiplication property of convolution. Now taking the inverse Fourier transform
% \begin{align}
% 	z &= \mathcal{F}^{-1}\left\{ \exp(i\boldsymbol{\nu}^{\top}\boldsymbol{\tau})W(\boldsymbol{\nu})\bar{Y}(\boldsymbol{\nu}) \ast \bar{Y}(\boldsymbol{\nu})\right\} \\
% 	&= w(\mathbf{r}+\boldsymbol{\tau}) \ast \bar{y}^2(\mathbf{r}) \\
% 	&= \int_{\Omega}{w(\mathbf{r}+\boldsymbol{\tau},\mathbf{r}')\bar{y}^2(\mathbf{r}')d\mathbf{r}'}
% \end{align}
% 
% 
% This may hold:
...
% \begin{align}
% 	R_{y_{t},y_{t}}(\boldsymbol{\tau}) &= \mathbf{E}\left[\left(\int_{\Omega}{ m\left(\mathbf{r}_n + \boldsymbol{\tau} - \mathbf{r}'\right) v_t\left(\mathbf{r}'\right)\, d\mathbf{r}'} + \boldsymbol{\varepsilon}_t\left(\mathbf{r}_n+\boldsymbol{\tau}\right)\right) \left(\int_{\Omega}{ m\left(\mathbf{r}_n - \mathbf{r}''\right) v_{t}\left(\mathbf{r}''\right) \, d\mathbf{r}''} + \boldsymbol{\varepsilon}_{t}\left(\mathbf{r}_n\right)\right) \right]\\
...
\mike{A further extension of the framework is to allow for the algorithm to learn online. The estimation framework that we have presented is inherently a batch algorithm designed to operate after data has been collected. A filter that designed to be implemented in an online setting, can be used in place of the smoother. Doing this, while sacrificing accuracy, will increase the speed at which the state estimation can be done and decrease the amount of information that would need to be stored. Approaches for online state estimation include the incremental EM algorithm \cite{Neal1998}, and the stepwise EM \cite{Sato00,Cappe09} may be used. These algorithms would facilitate dealing with non-stationary responses as well as providing a basis for optimal, model-based control, suggesting novel approaches to therapy. Something something something estimator, something something something online, something something something complete... about updating parameters rather than finding brand new ones, recursive LS estimator \cite{Ljung1999},}
...
The preservation of the system's stability, identifiability, observability, and controllability properties is not guaranteed with the discretization. Observability is assumed and then verified with the estimation results. The stability of the discretized IDE is proven below. To do this, the model is written as the series
\begin{align}
...
\end{equation}
Now, $g_t(\mathbf r)$ is always bounded since $0 \leq f(v_t(\mathbf r)) \leq 1$ and the area under the kernel is finite. To assess the stability of the discrete time system we assume that $g_t(\mathbf{r})$ is maximally saturated over the entire field for all time, which we denote as $g_{max}$. Substituting this into equation~\ref{eq:fieldmodelseries} yields
...
\end{align}
To study the asymptotic behaviour of the system we consider the limit as $t$ approaches infinity and in doing so we use a property of the geometric series to simplify equation \eqref{vrecursiveinlimit} to
