...
\begin{align}
	...
	&+ \xi\boldsymbol{\phi}^{\top}(\mathbf{r})\mathbf{x}_t + \mathbf{e}_t(\mathbf{r}).
\end{align} 
The Galerkin projection finds $\mathbf{x}_t$ such that for all basis functions, the inner product with the residual satisfies
\begin{equation}\label{eq:GalerkinProjection}
	G_i = \int_{\Omega}\Xi(x_t,v_t(\mathbf{r})) \phi_i(\mathbf{r}) d\mathbf{r} = 0, \> i=0,1,\hdots,n_x-1,
\end{equation}
