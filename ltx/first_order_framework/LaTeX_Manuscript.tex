...
% 	$f_{max}$ & maximal firing rate & 10 & spikes.s$^{-1}$\\
	...
	$\sigma_{\phi}$ & width of field basis functions in spatial domain & 2.5 & mm$^2$\\
	\hline 
...
This section gives the derivation for the width of the Fourier transform of the field basis function in terms of its spatial cut-off frequency (equations~\ref{eq:GaussianFT} and~\ref{eq:WidthFrequencyRelationship}). We define an \textit{n}-dimensional Gaussian  centerd at the origin as 
\begin{equation}
 \phi(\mathbf r)=\mathrm{exp}\left(-\frac{1}{\sigma_{\phi}^2}\mathbf r^\top\mathbf r\right).
\end{equation}
...
\end{equation}
A Gaussian basis function is symmetric with respect to its center so
