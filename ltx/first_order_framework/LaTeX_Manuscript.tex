...
\begin{align}
	...
	&= \mathbf{E}\left[\mathcal{F}^{-1} \left\{e^{i\boldsymbol{\nu}^{\top}\boldsymbol{\tau}} M(\boldsymbol{\nu}) M(\boldsymbol{\nu}) V(\boldsymbol{\nu}) \ast V(\boldsymbol{\nu}) \left(\xi  + T_s \varsigma W(\boldsymbol{\nu}) \right) \right\} \right] \\
	&= \mathbf{E}\left[m(\mathbf{r} + \boldsymbol{\tau}) \ast m(\mathbf{r}) \ast v^2(\mathbf{r}) \ast \left(\xi  + T_s \varsigma w(\mathbf{r})\right) \right] \\
	&= \left(R_{y_t,y_t}(\boldsymbol{\tau}) - \delta(\boldsymbol{\tau}\right)\sigma_{\varepsilon}^2) \ast \left(\xi  + T_s \varsigma w(\mathbf{r})\right) \label{eq:xcorr_with_autocorr}
\end{align}
where $R_{y_{t},y_{t}}(\boldsymbol{\tau})$ is the auto-correlation of the observations and $\delta(\cdot)$ is the Kronecker delta function.
...
\begin{align}
	...
	&= \mathbf{E}\left[m(\mathbf{r} + \boldsymbol{\tau}) \ast v(\mathbf{r}) \times m(\mathbf{r}) \ast v(\mathbf{r})\right] + \delta(\boldsymbol{\tau})\sigma_{\varepsilon}^2 \\
	&= \mathbf{E}\left[\mathcal{F}^{-1}\left\{ \exp(i\boldsymbol{\nu}^{\top}\boldsymbol{\tau})M(\boldsymbol{\nu})V(\boldsymbol{\nu}) \ast M(\boldsymbol{\nu})V(\boldsymbol{\nu}) \right\}\right] + \delta(\boldsymbol{\tau})\sigma_{\varepsilon}^2 \\
	&= \mathbf{E}\left[\mathcal{F}^{-1}\left\{ \exp(i\boldsymbol{\nu}^{\top}\boldsymbol{\tau})M(\boldsymbol{\nu})M(\boldsymbol{\nu})V(\boldsymbol{\nu}) \ast V(\boldsymbol{\nu}) \right\}\right] + \delta(\boldsymbol{\tau})\sigma_{\varepsilon}^2 \\
	&= \mathbf{E}\left[m(\mathbf{r}+\boldsymbol{\tau}) \ast m(\mathbf{r}) \ast v^2(\mathbf{r})\right] + \delta(\boldsymbol{\tau})\sigma_{\varepsilon}^2
\end{align}

Now to isolate the kernel, the Fourier transform is taken
\begin{align}
	\mathcal{F}\left\{R_{y_{t},y_{t+1}}(\boldsymbol{\tau})\right\} &\approx \mathcal{F}\left\{ \left(R_{y_t,y_t}(\boldsymbol{\tau}) - \delta\left(\boldsymbol{\tau}\right)\sigma_{\varepsilon}^2\right) \ast \left(\xi  + T_s \varsigma w(\mathbf{r})\right) \right\} \\
	&= \mathcal{F}\left\{ R_{y_t,y_t}(\boldsymbol{\tau}) - \delta\left(\boldsymbol{\tau}\right)\sigma_{\varepsilon}^2 \right\} \mathcal{F}\left\{ \xi  + T_s \varsigma w(\mathbf{r}) \right\}.
\end{align}
Now rearranging
\begin{equation}
	\frac{\mathcal{F}\left\{R_{y_{t},y_{t+1}}(\boldsymbol{\tau})\right\}}{\mathcal{F}\left\{ R_{y_t,y_t}(\boldsymbol{\tau}) - \delta\left(\boldsymbol{\tau}\right)\sigma_{\varepsilon}^2 \right\}} \approx \mathcal{F}\left\{ \xi  + T_s \varsigma w(\mathbf{r}) \right\}.
\end{equation}
Take inverse Fourier transform and rearrange to get an approximation of the kernel
\begin{align}
	\mathcal{F}^{-1}\left\{\frac{\mathcal{F}\left\{R_{y_{t},y_{t+1}}(\boldsymbol{\tau})\right\}}{\mathcal{F}\left\{ R_{y_t,y_t}(\boldsymbol{\tau}) - \delta\left(\boldsymbol{\tau}\right)\sigma_{\varepsilon}^2 \right\}}\right\} &\approx \xi  + T_s \varsigma w(\mathbf{r}) % \\
	 % 	(\varsigma T_s)^{-1} \mathcal{F}^{-1}\left\{\frac{\mathcal{F}\left\{R_{y_{t},y_{t+1}}(\boldsymbol{\tau})\right\}} {\mathcal{F}\left\{ R_{y_t,y_t}(\boldsymbol{\tau}) - \delta\left(\boldsymbol{\tau}\right)\sigma_{\varepsilon}^2 \right\}}\right\} - (\varsigma T_s)^{-1}\xi &\approx w(\mathbf{r})
\end{align}
