...
\begin{eqnarray}
	...
	&\times& d\mathbf{r}' d\mathbf{r} \boldsymbol{\theta}  
	 ...
	 % \end{split}
\end{eqnarray}
The analytic derivation of the inner product of \emph{n}-dimensional Gaussians, which was used for calculating $\mathbf\Gamma$ is provided in the supplementary material. Equation~\ref{eq:ReducedForm} can be simplified by exploiting the isotropy of the connectivity kernel basis functions where
\begin{equation}
...
\end{equation}
where $\left[ \boldsymbol\Psi(\mathbf{r}')\right]_{:i}$ denotes the $i^{th}$ column of $\boldsymbol{\Psi}(\mathbf{r}')$ which is a constant $L \times n_{\theta}$ matrix, where $L$ is the number of basis functions (and states) and $n_{\theta}$ is the number of connectivity kernel basis functions. This matrix is defined analytically (see the supplementary (\dean{Appendix}) material for analytic convolution of two Gaussians). Substituting equation~\ref{eq:DefPsi} into equation~\ref{eq:ReducedForm} gives
\begin{eqnarray}
