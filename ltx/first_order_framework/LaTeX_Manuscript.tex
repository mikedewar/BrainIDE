...
\end{equation}
where $\boldsymbol{\psi}(\mathbf{r},\mathbf{r}')$ is a vector of Gaussian basis functions and $\boldsymbol{\theta}$ is a vector of scaling parameters. A graphical demonstration of the connectivity kernel decomposition is provided in Fig.~\ref{fig:Figure1}. By exploiting the isotropy of the connectivity basis functions, they can be written as  
...
A further extension of the framework is to allow for the algorithm to learn online. The estimation framework that we have presented is inherently a batch algorithm designed to operate after data has been collected. A simple real-time implementation can be achieved using the UKF and an online recursive LS estimator \cite{Ljung1999}. Other more sophisticated approaches that may be used in a real-time estimation scheme include the incremental EM algorithm \cite{Neal1998}, and the stepwise EM \citep{Sato2000,Cappe2009}.
