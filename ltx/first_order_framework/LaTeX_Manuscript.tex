...
\parham{(Reviewer1, Additional comments 2:) The preservation of the system's stability, identifiability, observability, and controllability properties is not guaranteed with the discretization. Observability is assumed and then verified with the estimation results. Here we show that if $0 < T_s < \tau$, where the synaptic time constant $\tau = 1/\zeta$,  the discretized IDE is stable. To do this, the model is written as the series
\begin{align}
