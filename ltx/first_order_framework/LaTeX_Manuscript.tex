...
\dean{To show how to solution of equation for the state vector is equivalent to the Galerkin projection we define the residual between the field and the decomposition as 
...
\end{equation}
where $i=0,1,\hdots,n_x-1,$ and $n_x$ is the number of states and basis functions. If the inner product between the residual and the basis functions in equation~\ref{eq:GalerkinProjection} is zero, then the basis functions are orthogonal to the residual. By substituting equation~\ref{eq:BasisFunctionResidual} into equation~\ref{eq:GalerkinProjection} and rearranging we get equation~\ref{eq:decompRHS} in the main text. Note, if Galerkin's method was used estimate the states, $n_x$ coupled ODEs would need to be solved independently (each $G_i$) to estimate the state vector, which is a standard method in fluid dynamics and economics modeling. The method in this paper takes a different approach by forming the state-space model, allowing for more efficient estimation. }
