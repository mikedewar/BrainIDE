...
Histograms of the parameter estimates obtained over the 150 realisations are shown in Figure~\ref{fig:Figure7}. The actual parameter values, the mean estimates, corresponding standard deviations and biases are given in Table~\ref{tab:Parameters estimates}. The biases show the percentage differences between the actual values and the mean parameter estimates. The mean estimates are in good agreement with the actual values of the connectivity kernel parameters, however the time constant parameter, $\xi$, is outside the estimate distribution (with bias of 2.78\%).
...
To study the robustness of the estimation algorithm, the performance was studied for various connectivity kernels. The reconstructed connectivity kernels from the estimated parameters are shown in Figure~\ref{fig:Figure8}. By changing the weights of the connectivity kernel we are effectively changing gain of the system and the range of the field amplitudes. Since the sigmoidal nonlinearity is remaining static, we are changing the operating region on the sigmoid. Therefore, we expect our estimation accuracy to vary. For each subset of parameters the mean of the reconstructed kernel over 50 realisations is shown as a dotted line and the 95\% confidence interval is indicated by the grey shaded region. Parts a), b) and c) of Figure~\ref{fig:Figure8} show results from the Mexican hat kernel parameters were varied. In all cases the actual kernel fell within the confidence interval of the estimated kernel. The kernel in Part a) has the least amount of excitation and the most amount of inhibition. Therefore of the three Mexican hat kernels shown, this one will have the least amount of activation. The result is an overestimate of the central excitatory region of the kernel. Conversely, in Part c) of the figure the actual kernel has the most amount of excitation and the least amount of inhibition. Therefore 

The mean kernel estimate for Part b) shows very good accordance with the actual kernel. In the other Parts a) and c) there is an over estimate and an underestimate of the kernel, respectively. These bias are a result 
