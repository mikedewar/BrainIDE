...
Histograms of the parameter estimates obtained over the 150 realisations are shown in Figure~\ref{fig:Figure7}. In each of the subplots of the figure the true parameter value is shown by the solid line and the mean parameter estimate is shown by the dashed line. The actual parameter values, the mean parameter estimates, corresponding standard deviations, and biases for this parameter combination are given as the first entry in Table~\ref{tab:MexicanHatKernelResults} with results from Experiment II. \dean{We need to switch the graphs in Figure 10 so that the first one uses these parameters} \parham{it's in ascending order for $\theta_0$ for both rows}. The biases show the percentage differences between the actual values and the mean parameter estimates. For this parameter combination the mean estimates are in good agreement (within 1 standard deviation) with the actual values of the connectivity kernel parameters. However, the time constant parameter, $\xi$, is outside the estimate distribution (with bias of $2.56\%$), with a small variance in parameter estimates.
