...
\begin{equation}
	...
	\, d\mathbf{r}'dt'.
\end{equation}
To rearrange the model into an integro-differential equation the Laplace transform of equation~\ref{FullDoubleIntModel} is taken, giving
\begin{align}
	V(s) &= H(s)G(s) \\
	&=\frac{G(s)}{s+\alpha},
\end{align}
since $H(s) = (s+\alpha)^{-1}$. After rearranging we can write
\begin{equation}
	(s+\alpha)V(s) = G(s).
\end{equation}
