...
\begin{document}
    ...
    \begin{enumerate}
		...
		\emph{\parham{ discretized membrane voltage $v_{t+1}(\mathbf r)$ can be written in a form of 
...
\mathbf{r}'\right) \right)d\mathbf{r}'} 
		  ...
		  \end{align}
		  in a limit equation (geometric series) \eqref{vrecursiveinlimit} can be written as 
		...
		\emph{We should add some more to the modeling assumptions section to address this one. I have begun to talk about the LL method. \parham{here he just ask about Euler method he doesn't ask about s-word.}}
