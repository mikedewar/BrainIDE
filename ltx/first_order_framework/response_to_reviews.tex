...
\mathbf{r}'\right) \right)d\mathbf{r}'} 
		  \end{equation}
		  in a worst case scenario if we assume $f(v_t(\mathbf r))$ saturates for all $g_t$ we have
		  \begin{align}
		   g_{t}(\mathbf r)=&\int_\Omega  {w\left( \mathbf{r}-\mathbf{r}' \right)d\mathbf{r}'} \nonumber \\
				   =-a
		  \end{align}	
		  where $a$ is the area under the connectivity kernel. Substituting $-a$ for all $g_{t}$ in \eqref{vrecursive} we get
		  \begin{align}\label{vrecursiveinlimit}
		   v_{t+1}(\mathbf r)&=\xi^{t+1}v_0(\mathbf r)-aT_s\sum_{i=0}^t\xi^i \nonumber\\
				     &=\xi^{t+1}v_0(\mathbf r)-\frac{aT_s(1-\xi^{t+1})}{1-\xi}
		  \end{align}
		  in a limit equation \eqref{vrecursiveinlimit} can be written as 
		  \begin{equation}\label{simplifiedvrecursiveinlimit}
		   v_{t+1}(\mathbf r)=\xi^{t+1}v_0(\mathbf r)-\frac{aT_s}{1-\xi}
		  \end{equation}
		  substituting $\xi=1-\zeta T_s$ in \eqref{simplifiedvrecursiveinlimit}
		  \begin{equation}
		   v_{t+1}(\mathbf r)=(1-\zeta T_s)^{t+1}v_0(\mathbf r)-\frac{a}{\zeta}
		  \end{equation}
		  by choosing sufficiently small $T_s$ the discretized model will be stable.}}
