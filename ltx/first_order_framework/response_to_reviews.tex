...
\begin{document}
	...
	\emph{The examples presented in the paper use the Mexican hat connectivity kernel. The framework is also valid for other connectivity kernels provided they are spatially homogeneous and that they can be approximated by a set of weighted isotropic basis functions. The mathematical formulation does not \mike{place any further restrictions on the form of the spatial mixing kernel.} \st{restrict the shape of the kernel, where anisotropic (we assume isotropy of the kernel basis functions to make the simplification in equation~18) arbitrary shapes can be represented.} \mike{Does any of the rest of this answer address the question?} Furthermore, the kernel basis functions need not be a Gaussian shape, but can be, for example, Laplacian to describe the ``wizard hat'' connectivity or B-spline functions to describe a kernel with a compact support. For more complex basis functions some of the analytic derivations described in the Appendix may have to be performed numerically. We have added the above statements to the methods section (after equation~13) to be more explicit in describing subtleties of the method.}
...
\emph{Fourier basis functions are indeed a correct alternative in the case examined in this paper. In fact Fourier basis functions or any other orthonormal basis functions simplify the state space representation of the model, since $\Gamma$ defined in equation (20) of the manuscript becomes Identity matrix. However, we have chosen Gaussian basis functions due to their semi-compact support. By choosing Gauassian basis functions the stat space representation of the model remains in its generic form which is also correct in the case of orthonormal basis functions.}
...
\emph{A few comments have been added to the Modeling Assumptions section of the discussion.}
