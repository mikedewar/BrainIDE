...
\begin{equation}
	...
	\frac{dv\left( \mathbf{r},t \right)}{dt} = {\color{purple}-\frac{1}{\tau} v\left( \mathbf{r},t \right)} +\int_\Omega {{\color{orange}w\left( \mathbf{r},\mathbf{r}' \right)}{\color{blue}f\left( {v\left( \mathbf{r}',t \right)} \right)}\, d\mathbf{r}'}
\end{equation}
\end{frame}
...
\begin{frame}\frametitle{Observation equation}
	...
	\begin{equation}\label{eq:ObservationEquation}
		y_t(\mathbf{r}_n) = \int_{\Omega} { {\color{cyan}\underbrace{m\left(\mathbf{r}_n-\mathbf{r}'\right)}_{\text{sensor}}} v_t\left(\mathbf{r}'\right) \, d\mathbf{r}'} + {\color{magenta}\underbrace{\varepsilon_t(\mathbf{r}_n)}_{\text{noise}}}, 
	\end{equation}
	where
	\begin{align}
...
\begin{frame}\frametitle{Model output}
	\begin{align}
		...
		y_t(\mathbf{r}_n) &= \int_{\Omega} { {\color{cyan}m\left(\mathbf{r}_n-\mathbf{r}'\right)} v_t\left(\mathbf{r}'\right) \, d\mathbf{r}'} + {\color{magenta}\varepsilon_t(\mathbf{r}_n)} \nonumber 
	\end{align}
\begin{center} 
...
\begin{equation}
	\boldsymbol{\phi}^{\top}(\mathbf{r})\mathbf{x}_{t+T_s} = 
...
This simplifies to
\begin{align}
	\mathbf{x}_{t+1} = {\color{purple}\xi\mathbf{x}_t} 
...
\begin{frame}\frametitle{Reduced observation equation}
	\begin{equation}
		y_t(\mathbf{r}_n) = \int_{\Omega} { {\color{cyan}m\left(\mathbf{r}_n-\mathbf{r}'\right)} \boldsymbol{\phi}^{\top}(\mathbf{r})\mathbf{x}_{t} \, d\mathbf{r}'} + {\color{magenta}\varepsilon_t(\mathbf{r}_n)} 
	\end{equation}
\pause
...
Aim: To track the patient-specific neural dynamics and estimate the parameters that influence seizures from electrophysiological measurements.	
	...
	\end{align}
	\pause
	We need to estimate the states, $\mathbf{x}_t$, and parameters, $\boldsymbol\theta$ and $\xi$, given the observations, $\mathbf{y}_t$.
\end{frame}
...
\begin{frame}\frametitle{Estimation algorithm}
	...
	% \end{align}
	% \pause
We need to estimate the states, $\mathbf{x}_t$, and parameters, $\boldsymbol\theta$ and $\xi$, given the observations, $\mathbf{y}_t$. (see refs\footnote{Ljung, L., (1999); Haykin, S., (2001); Sarkka, S., (2010); Julier, S. and Uhlmann, J., (1997).})
\begin{center}
...
Let
	\begin{equation}
		\mathbf{q}(\hat{\mathbf{x}}_t) = \int_\Omega \boldsymbol{\Psi}(\mathbf{r}') f(\boldsymbol{\phi}^{\top}(\mathbf{r}')\hat{\mathbf{x}}_t) \,\textrm{d}\mathbf{r}'.
	\end{equation}

Given a state estimate sequence we can write
\begin{align*}
...
\begin{frame}\frametitle{Parameter estimation 2}
...
\begin{frame}\frametitle{State estimation}
