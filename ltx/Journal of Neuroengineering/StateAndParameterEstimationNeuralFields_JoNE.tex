...
\end{eqnarray}
where $T_s$ is the time step, $\xi = 1-\zeta T_s$ and $e_t(\mathbf{r})$ is i.i.d. such that $e_t\sim\mathcal{GP}(\mathbf 0,\gamma(\mathbf{s}-\mathbf{r}))$. \haq{Can we remove noise form 8 and introduce only discretization there; then introduce noise in another equation.} This term was added to account for uncertainty in the model. To represent the iEEG/LFP data collection precedure we used the output function 
\begin{equation}
