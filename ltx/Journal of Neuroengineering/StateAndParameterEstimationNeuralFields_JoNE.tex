...
\begin{figure}[ th]\label{fig:2d_kernel}
   	\begin{center}
   		\includegraphics[angle = 90, scale=0.5]{./Graph/2D_kernel.pdf} 
   	\end{center}
...
\begin{equation}
	\mathbf{x}_{t+1} = \int_\Omega \boldsymbol{\Psi}(\mathbf{r}') f(\boldsymbol{\phi}^{\top}(\mathbf{r}')\mathbf{x}_t) d\mathbf{r}' \boldsymbol{\theta} + \xi\mathbf{x}_t +\mathbf{e}_t.
\end{equation}
\begin{equation} 
	\mathbf{y}_t = \mathbf{C}\mathbf{x}_t + \boldsymbol{\varepsilon}_t,
\end{equation}

This reduced model can be manipulated into a standard state-space model in the form (see \ref{Simplifying Decomposition})
\begin{eqnarray}
...
Q\left( \mathbf{x}_t \right) &=& q(\mathbf{x}_t)\boldsymbol{\theta} - \xi\mathbf{x}_t \label{eq:QmatrixForSigmapoints}\\
	...
	\boldsymbol{\Gamma} &=& \int_\Omega {\boldsymbol{\phi} \left(\mathbf{r}\right)\boldsymbol{\phi} ^{\top}\left(\mathbf{r}\right)d\mathbf{r}}, \label{DefGamma} 
\end{eqnarray}
...
\end{equation}
Since the observation equation is linear (equation~\ref{ObservationEquation}) the standard Kalman Filter update equations are  used to correct the predicted states. The state estimates from the forward filtering are used to form a new set of sigma points for the smoother, as described above. To compute the smoother gain, the predicted cross-covariance matrix of the states is used with the covariance matrix. A summary of the URTSS procedure is given in Algorithm~\ref{UKFAlgorithm}. It should be noted that  the disturbance, $e_t\left(\mathbf{r}\right)$ and the measurement noise, $\boldsymbol{\varepsilon}_t$, are additive. Therefore, the additive form of the URTS is used, otherwise the state vector should be augmented with the noise terms. The additive disturbance form reduces the dimension and the number of sigma points used in the algorithm.
\begin{algorithm}
	\begin{small}
...
\end{figure}
\begin{figure}[th]
\subfigure[$\zeta$ convergence]{\includegraphics[width=1.5in]{./Graph/ZetaConvergence.pdf}}
\subfigure[][]{\includegraphics[width=0.24\textwidth]{./Graph/theta0Convergence.pdf}}\\
\subfigure[][]{\includegraphics[width=0.24\textwidth]{./Graph/theta1Convergence.pdf}}
\subfigure[][]{\includegraphics[width=0.24\textwidth]{./Graph/theta2Convergence.pdf}}
...
\begin{figure}
	...
	\begin{center}
\subfigure[][]{\includegraphics[scale=0.8]{./Graph/BasisChar.pdf}}
\subfigure[][]{\includegraphics[scale=0.8]{./Graph/SensorChar.pdf}}
\subfigure[][]{\includegraphics[width=1.8in]{./Graph/ObservationFrequencyResponse.pdf}}\\
	\end{center}
...
\begin{figure*}
\subfigure[][]{\includegraphics{./Graph/KernelEstimate_skinny.pdf}}
\subfigure[][]{\includegraphics{./Graph/KernelEstimateFFT_skinny.pdf}}\\
\subfigure[][]{\includegraphics{./Graph/KernelEstimate_wide.pdf}}
\subfigure[][]{\includegraphics{./Graph/KernelEstimateFFT_wide.pdf}}\\
\subfigure[][]{\includegraphics{./Graph/KernelEstimateFFT_skinny_db.pdf}}

\caption{Results for connectivity estimate. In all the subplots the true kernel is show by the solid line, the mean kernel estimate (over 250 realisations) is shown by the dashed line and the 95~\% confidence interval is shown by the shaded grey region. \textbf{a.} Comparison of kernel estimates to true kernel in the spatial domain. \textbf{b.} Comparison of the true kernel to the estimated kernel in the frequency domain. \dean{Note: needs to be changed to absolute values if we include it.} \textbf{c.} Same as part a., except narrower wider (in space) basis functions where used in the model reduction. \textbf{d.} Same as part b., except narrower wider (in space) basis functions where used in the model reduction. \textbf{e.} Frequency domain (power dB) representation of the true kernel and the estimated kernel (using the narrower basis functions). \dean{Note: Power and dB needs to be non-italicised.}}
...
\begin{figure}[th]
\centering
\subfigure[][]{\label{fig:FieldFFT}\includegraphics[scale=0.9]{./Graph/FFTTrueField.pdf}}
\subfigure[][]{\label{fig:EstimatedFieldFFT}\includegraphics[scale=0.9]{./Graph/FFTEstimatedField.pdf}}\\
...
\begin{figure*}[!th]
\centering 
\subfigure[][]{\includegraphics[scale=0.8]{./Graph/TrueField90.pdf}}
\subfigure[][]{\includegraphics[scale=0.8]{./Graph/EstimatedField90.pdf}}
\subfigure[][]{\includegraphics[scale=0.8]{./Graph/EstimationError90.pdf}}\\
...
The finite state space model given in \ref{eq:ReducedForm} is linear with respect to parameters. Therefore a least square (LS) method can be applied to estimate the unknown parameters. We define $L \times n_{\theta}$ matrix
\begin{equation}
	\mathbf{q}(\mathbf{x}_t) = \int_\Omega \boldsymbol{\Psi}(\mathbf{r}') f(\boldsymbol{\phi}^{\top}(\mathbf{r}')\mathbf{x}_t) d\mathbf{r}' \boldsymbol{\theta}
\end{equation}
Given a state estimate sequence from the unscented Rauch Tung Striebel smoother (URTSS), we can write
\begin{eqnarray}
