...
\end{equation}
We will proceed in the derivation of the convolution assuming the field basis function is positioned at the origin ($\boldsymbol\mu_i=\mathbf 0$). The convolution theorem states that the Fourier transform of a convolution is the product of Fourier transforms. Using the derivation of the Fourier transform of a \emph{n}-dimensional Gaussian from equation~\ref{eq:FTofGaussian} of \ref{ap:FrequencyAnalysis} we have
\begin{eqnarray}
...
\begin{equation}\label{eq:ConvoftwoGaussian}
 \phi_i(\mathbf{r})*\psi_j(\mathbf{r}'-\mathbf{r}) = \left(\frac{\pi\sigma_{\phi}^2\sigma_{\psi_j}^2}{\sigma_{\phi}^2+\sigma_{\psi_j}^2}\right)^{\frac{n}{2}}\mathrm{exp}\left({-\frac{1}{\sigma_{\phi}^2+\sigma_{\psi_j}^2} \mathbf r^\top\mathbf r}\right).
\end{equation}
...
\begin{equation}\label{eq:ConvoftwoGaussianNonzeroMean}
 (\phi_i*\phi_j)(\mathbf r)=\left(\frac{\pi\sigma_{\phi}^2\sigma_{\psi_j}^2}{\sigma_{\phi}^2+\sigma_{\psi_j}^2}\right)^{\frac{n}{2}}\mathrm{exp}\left({-\frac{1}{\sigma_{\phi}^2+\sigma_{\psi_j}^2} (\mathbf r-\boldsymbol\mu_i)^\top(\mathbf r-\boldsymbol\mu_i)}\right).
\end{equation}
