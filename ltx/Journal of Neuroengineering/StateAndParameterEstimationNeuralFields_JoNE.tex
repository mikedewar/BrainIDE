...
\subsection{Derivation of Finite Dimensional State-Space Model}\section{Sect:ReducedModelDerivation}
...
\begin{figure}
   	...
   	\end{center}
   	\caption{An example of continuous field decomposed by a finite number of basis functions scaled by the state vector. } 
...
\end{figure}
The accuracy of the state estimates was evaluated by comparing the field reconstruction to the true field using the mean (over iterations) of the root mean square error (MRMSE) (over space). The RMSE of the field estimates over time for 100 realisations is shown in Figure~\ref{fig:RMSE}, with the $95~\%$ confidence interval. At each time point the error is consistently around 0.5, which approximately $7.1$~\% of the field voltage range, which is $(-3.29,3.77)$~mV. The error in the reconstructed field is mostly due to the loss of high spatial frequency information due to the basis function decomposition as illustrated in Figure~\ref{fig:FieldEstimate}. The figure shows a snapshot at a single time instant of the true neural field, the estimated field and the corresponding error, which is the difference between the true and estimated neural fields. It can be seen in Figure~\ref{fig:FieldEstimate}(b) that the estimated field contains the basic structure of the true field (Figure~\ref{fig:FieldEstimate}(a)), but is missing the higher-frequency detail.
  \begin{figure}
   	\begin{center}
...
\begin{lemma}
	...
	\begin{equation}\label{eq:AppendixWt} 
		\mathbf e_t(\mathbf r)=\boldsymbol{\Gamma}^{-1}\int_\Omega {\boldsymbol{\phi} ( \mathbf{r} )e_t( \mathbf{r} )d\mathbf{r}} 
	\end{equation}
...
\end{lemma}
\section*{Proof} Equation (\ref{eq:AppendixWt}) is a linear function of $e_t(\mathbf r)$ and hence $\mathbf{e}_t$ is also normally distributed. The expected value of $\mathbf e_t(\mathbf r)$ is given by 
\begin{eqnarray}
	\mathbf E\left[ \mathbf e_t(\mathbf r)\right]&=& \mathbf{\Gamma}^{-1}\int_{\Omega}\boldsymbol\phi\left(\mathbf{r}\right)\mathbf E\left[e_t\left(\mathbf{r}\right)\right] d\mathbf{r} \nonumber \\
	&=&\mathbf 0 
\end{eqnarray}
The covariance of $\mathbf{e}_t(\mathbf r)$ is 
\begin{eqnarray}
	\lefteqn{\mathbf E\left[ \mathbf e_t(\mathbf r) \mathbf e_t^\top(\mathbf r')\right]} \nonumber \\ 
...
\end{equation}
For $3$~dB attenuation at $\boldsymbol\nu_c$ we need to set
\begin{eqnarray}
...
\end{equation}
\section{Convolution of two $n$-dimensional Gaussian basis functions}\label{ap:ConvOfGaussians}
...
\begin{eqnarray}
\mathcal{F}\left(\phi_i*\psi_j\right)(\mathbf{r}) &=& \left(\pi\sigma_{\phi}^2\sigma_{\psi_j}^2\right)^{\frac{n}{2}}\left(\frac{1}{\sigma_{\phi}^2+\sigma_{\psi_j}^2}\right)^{\frac{n}{2}} \nonumber \\
...
\begin{equation}\label{eq:ConvoftwoGaussian}
 \left(\phi_i*\psi_j\right)(\mathbf{r}) = \left(\frac{\pi\sigma_{\phi}^2\sigma_{\psi_j}^2}{\sigma_{\phi}^2+\sigma_{\psi_j}^2}\right)^{\frac{n}{2}}\mathrm{exp}\left({-\frac{1}{\sigma_{\phi}^2+\sigma_{\psi_j}^2} \mathbf r^\top\mathbf r}\right).
\end{equation}
...
\begin{equation}\label{eq:ConvoftwoGaussianNonzeroMean}
 \left(\phi_i*\psi_j\right)(\mathbf{r}) = \left( \frac{\pi\sigma_{\phi}^2\sigma_{\psi_j}^2} {\sigma_{\phi}^2+\sigma_{\psi_j}^2}\right)^{\frac{n}{2}} \mathrm{exp}\left({-\frac{1}{\sigma_{\phi}^2+\sigma_{\psi_j}^2} (\mathbf r-\boldsymbol\mu_i)^\top(\mathbf r-\boldsymbol\mu_i)}\right).
\end{equation}
The analytic definition of the observation matrix, $C$, can be derived in a similar fashion, replacing connectivity kernel basis with the sensor.  
\section{Inner Product Bit}\label{ap:InnerProdOfGaussians}
...
\Gamma_{i,j} = \phi_i\otimes\phi_j=\int_{\mathbb{R}^n}\phi_i(\mathbf r)\phi_j(\mathbf r)d\mathbf{r}.
\end{equation}
...
\begin{eqnarray}
\phi_i\otimes\phi_j&=&\int_{\mathbb{R}^n}\phi_i(\mathbf r)\phi_j(\mathbf r)d\mathbf r\\
&=&\int_{\mathbb{R}^n}\phi_i(2\boldsymbol \mu_i-\mathbf r)\phi_j(\mathbf r)d\mathbf r\\
