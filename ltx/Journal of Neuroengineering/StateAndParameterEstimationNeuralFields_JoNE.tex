...
\end{equation} 
where $T$ is the time step, $\xi = 1-\zeta T$ and is an iid spatially colored, temporally white disturbance term with the covariance 
\begin{equation}
...
\begin{equation} 
\boldsymbol{\Sigma}_e=\boldsymbol{\Gamma}^{-1}\int\limits_{\Omega}\int\limits_{\Omega}\boldsymbol{\phi}\left(\mathbf r\right) \gamma\left(\mathbf{r}- \mathbf{r}' \right)\boldsymbol{\phi}\left(\mathbf{r}'\right)^{\top}d\mathbf{r}' d\mathbf{r}\boldsymbol{\Gamma}^{- \top} 
\end{equation}
...
The approximation of the state density is made by propagating the so called sigma points through the state-space model. The sigma points describe the mean and covariance of the state estimate. The number of sigma points needed is $2n_x+1$ where $n_x$ is the dimension of the stae space. The set of sigma points and their corresponding weights used in the UKF can be calculated as
