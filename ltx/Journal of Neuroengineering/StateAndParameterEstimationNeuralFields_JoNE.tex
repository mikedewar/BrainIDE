...
In this section we describe the procedure for estimating the states that scale the field basis functions, the connectivity kernel parameters and the synaptic time constant. The estimation process consists of a two part iterative algorithm, consisting of the state estimation step followed by the parameter estimation step. The two step estimation algorithm exploits the fact the parameters are linear with respect to the state estimate. Therefore, the parameter estimation step utilises a least squares (LS) method that minimises the sum of the squared errors (of a predicted state update) with each new state estimate (see Appendix~\ref{LeastSquaresAppendix}). In the state estimation step, given an estimate of $\boldsymbol \theta$ and $\xi$ the Unscented Rauch Tung Striebel Smoother (URTSS) \cite{Sarkka2010} was used to determine the posterior state sequence .
