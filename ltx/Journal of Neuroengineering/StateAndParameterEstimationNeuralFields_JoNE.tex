...
\end{equation}
Using the same neural field basis function approximation, the observation equation of the reduced model is found by substituting equation~\ref{DefFieldDecomp} into~\ref{eq:ObservationEquation} giving
...
Such spectral considerations also facilitate the choice of the basis function width parameter~\cite{Sanner1992,Scerri2009}. To ensure that the basis functions can represent fields with frequency content up to $\boldsymbol\nu_{cy}$, they should be chosen to poses a spectral bandwidth of at least $\boldsymbol\nu_{cy}$. This condition is satisfied by setting the width parameter such that the spectral response of the basis functions results in an attenuation of not more than 3dBs at $\boldsymbol\nu_{cy}$.

To demonstrate this we begin with the 2-dimensional Gaussian basis function defined in equation \ref{eq:FieldBasisFunction}, with the corresponding Fourier transform
...
\end{equation}
Derivations for equations~\ref{eq:GaussianFT} and \ref{eq:WidthFrequencyRelationship} are given in~\ref{ap:FrequencyAnalysis}.
...
\centering
\begin{tabular}{cccc}
	\hline\hline Parameter & Actual Value & Mean Estimate & Standard Deviation \\
	\hline\hline
 	$\xi$ & 0.9 & 0.925 & 0.004 \\
	$\theta_0$ & 10 & 10.295 & 2.879 \\
	$\theta_1$ & -8 & -8.571 & 1.996 \\
	$\theta_2$ & 0.5 & 0.551 & 0.083 \\
\end{tabular}
