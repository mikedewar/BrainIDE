...
\begin{figure}[th]\label{fig:experimental design}
   	\begin{center}
   		\includegraphics[width=0.3\textwidth]{./Graph/2D_kernel.pdf} 
   	\end{center}
...
\end{equation}
and manipulating equation \ref{reduced continuous model} (see \ref{Simplifying Decomposition}) we isolate the state vector to form a state-space model 
\begin{eqnarray}
...
\end{equation}
The basis function width, $\sigma^2_{\phi}$, should be chosen to give at least 3~dB attenuation at $\boldsymbol\nu_{co}$ giving \dean{need to rewrite eq~\ref{eq:WidthFrequencyRelationship} in terms of $\sigma_{\phi}$. At the mo it is writen in terms of $\sigma_{nu}$}
...
\end{figure}
Following this, the distance between basis function, $\Delta_b$, was set to $2.5$ and the basis function width, $\sigma_{\phi}^2$, was set to 3.6, 3.16 mm full width at half maximum. \mike{Analysis said 3.9 from Parham's derivation, but 3.6 still overlaps and allows more high frequency details to be represented.}
