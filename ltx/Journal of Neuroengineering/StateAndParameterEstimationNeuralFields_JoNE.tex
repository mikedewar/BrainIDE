...
The development of an estimation framework also has theoretical implications. The procedure has the potential to allow testing of hypotheses that have been generated in theoretical studies and to validate neural field models. For example, it has been hypothesised that so-called ``bump solutions'' are a possible mechanism for short term memory formation~\cite{Coombes2005}. Using the proposed model-based framework, we can reconstruct a neural field from data, and estimate parameters and check if they correspond to the theoretically derived parameters where these bump solutions can exist. In addition, the parameters and the state-space can be explored during other cognitive tasks and states of arousal such as during sleep and anaesthesia. 
...
\end{equation}
\section{Convolution and inner product of two $n$-dimensional Gaussian basis functions}\label{ap:ConvandIn}
Consider two Gaussian basis functions
\begin{equation}
 \phi_i(\mathbf r)=\mathrm{exp}\left({-\frac{1}{\sigma_i^2} (\mathbf r-\boldsymbol \mu_i)^\top(\mathbf r-\boldsymbol \mu_i})\right)
\end{equation}
\begin{equation}
\phi_j(\mathbf r)=\mathrm{exp}\left({-\frac{1}{\sigma_j^2} (\mathbf r-\boldsymbol \mu_j)^\top(\mathbf r-\boldsymbol \mu_j})\right)
\end{equation}
First we derive the convolution of two Gaussian basis functions assuming $\boldsymbol\mu_i=\boldsymbol\mu_j=\mathbf 0$. Convolution theorem states that the Fourier transform of a convolution is the product of Fourier transforms. Using equation \ref{eq:FTofGaussian} we have
\begin{eqnarray}
\boldsymbol\Phi_i(\boldsymbol\nu)\boldsymbol\Phi_j(\boldsymbol\nu)&=&\left(\pi\sigma_i^2\sigma_j^2\right)^{\frac{n}{2}}\left(\frac{1}{\sigma_i^2+\sigma_j^2}\right)^{\frac{n}{2}} \nonumber \\
&\times& (\pi(\sigma_i^2+\sigma_j^2))^{\frac{n}{2}}\mathrm{exp}\left(-(\sigma_i^2+\sigma_j^2)\pi^2\boldsymbol\nu^\top\boldsymbol\nu\right),
\end{eqnarray}
taking the inverse Fourier transform we can write the convolution as
\begin{equation}\label{eq:ConvoftwoGaussian}
 (\phi_i*\phi_j)(\mathbf r)=\left(\frac{\pi\sigma_i^2\sigma_j^2}{\sigma_i^2+\sigma_j^2}\right)^{\frac{n}{2}}\mathrm{exp}\left({-\frac{1}{\sigma_i^2+\sigma_j^2} \mathbf r^\top\mathbf r}\right).
\end{equation}
...
\begin{equation}\label{eq:ConvoftwoGaussianNonzeroMean}
 (\phi_i*\phi_j)(\mathbf r)=\left(\frac{\pi\sigma_i^2\sigma_j^2}{\sigma_i^2+\sigma_j^2}\right)^{\frac{n}{2}}\mathrm{exp}\left({-\frac{1}{\sigma_i^2+\sigma_j^2} (\mathbf r-\boldsymbol\mu)^\top(\mathbf r-\boldsymbol\mu)}\right).
\end{equation}
where $\boldsymbol\mu=\boldsymbol\mu_i+\boldsymbol\mu_j$.

The inner product of two Gaussian basis functions is defined as
\begin{equation}\label{eq:InofGaussianDefinition}
\phi_i\otimes\phi_j=\int_{\mathcal R^n}\phi_i(\mathbf r)\phi_j(\mathbf r)d\mathbf r
\end{equation}
...
\begin{eqnarray}
\phi_i\otimes\phi_j&=&\int_{\mathcal R^n}\phi_i(\mathbf r)\phi_j(\mathbf r)d\mathbf r\\
&=&\int_{\mathcal R^n}\phi_i(2\boldsymbol \mu_i-\mathbf r)\phi_j(\mathbf r)d\mathbf r\\
&=&(\phi_i*\phi_j)(2\boldsymbol \mu_i)
\end{eqnarray}
evaluation equation \ref{eq:ConvoftwoGaussianNonzeroMean} at $\mathbf r=2\boldsymbol\mu_i $ gives
\begin{equation}
