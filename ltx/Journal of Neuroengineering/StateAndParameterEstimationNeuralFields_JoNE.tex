...
\begin{algorithm}
	...
	\begin{algorithmic}[1] 
		\State Forward initialisation \quad $\hat{\mathbf x}_0, \mathbf P_0 $
		...
		\end{equation*}
		\State Backward iteration: for $t \in \left\lbrace T-1, \cdots, 0 \right\rbrace $ calculate the sigma points $\mathcal X_{i,t}^b$ using \ref{eq:sigmapoints1}-\ref{eq:sigmapoints3} and propogate them through the state equation
		\begin{equation*}
		...
		\end{equation*}
		\State Compute the smoother gain, the smoothed state and the smoothed covariance matrix
		\begin{equation*}
