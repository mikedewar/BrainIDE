...
The distribution of the estimated parameters is shown in Figure~\ref{fig:Parameters}. The estimates of the parameters form distributions that are approximately Gaussian. The mean estimates, corresponding standard deviations and biases are given in Table~\ref{tab:Parameters estimates}. Although the distributions are quite wide, the mean estimates are in good concordance with the actual values. It should be noted that when the excitatory  bases of the connectivity are under (over) estimated, the inhibitory basis is typically under (over) estimated as well. This is demonstrated in Figure~\ref{fig:ParametersRatio} where we show the distribution of the ratios of the parameter estimates. 
\begin{table}
\centering
\begin{tabular}{cccc}
...
\caption{\dean{need to put $\left| \right|$ around $\hat\theta_0/\hat\theta_1$ in xlabel in \textbf{a.}} Distribution of the connectivity kernel basis function parameter ratios over 100 realisations. \textbf{a.} The absolute value of the ratio of the local excitatory parameter to the surround inhibition parameter (true ratio is 1.25). \textbf{b.} The ratio of the local excitatory parameter to the longer range excitatory parameter (true ratio is 20). The dotted line shows the location of the true ratio in each case.}
\label{fig:ParametersRatio}
\end{figure}
The reconstructed connectivity kernel from the estimated parameters can be seen in Figure~\ref{fig:KernelEstimates} with the $95\%$ confidence interval. This Figure verifies the expected net shape of the estimated connectivity kernel is in good accordance to the true kernel.
\begin{figure}
    \centering
% \subfigure[][]{
...
\caption{Examples of the actual and estimated neural fields. \textbf{a.} The actual field at a single time instant from the full neural field model that was used to generate data. \textbf{b.} The reconstructed field of the reduced model, showing the effect of the basis function decomposition on the frequency content of the estimated field. \textbf{c.} The absolute error between the true and reconstructed fields. The error shows high frequency behaviour.}
