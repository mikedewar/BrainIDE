...
time discretisation must be performed. To do this we used a one-step Euler method, where equation~7 of the main text can be approximated by 
\begin{equation}
...
This section gives the derivation for the field basis function spatial cut-off frequency as a function of the basis function width (equations~34 and~37 in the main text). For we define an \textit{n}-dimensional Gaussian  centred at the origin as 
...
The derivation in this section was used for calculating the matrix $\Gamma$, from equation~16 in the main text. The inner product of the two \emph{n}-dimensional Gaussians (from equations~S1.14 and~S1.15) is defined as
