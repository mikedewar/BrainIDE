...
With recent advances in medical imaging techniques and computing power we are entering a new age of neuroscientific research, where the amount of patient specific information is becoming incredibly detailed. In order to make the best use of this information, patient specific mathematical models should should be formed. This is particularly poignant where considering poorly understood complex systems such as large scale dynamics of the human brain. In terms of brain dynamics, there currently exists a library of neural field models that provide a theoretical explanation of various phenomena associated with various cognitive states and disease. In this paper we have created a platform for creating a patient specific neural field model. Within this platform, we propose rules for designing experiments in order to acquire the data with sufficient information and the mathematical formula to create such a model. 
...
\begin{figure}
   	...
   	\end{center}
   	\caption{{\bf Mexican-hat connectivity kernel, $w(\mathbf{r},\mathbf{r'})$}. The function $w(.)$ is rotationally symmetric (isotropic) about zero, hence a cross-section captures the important aspect of the connectivity kernel's shape. The kernel decays asymptotically to zero.}
 	\label{fig:2d_kernel}
   \end{figure}
...
\begin{figure}
   	...
   	\end{center}
   	\caption{{\bf Example of a field decomposition.} An example of a continuous one-dimensional field decomposed by a finite number of basis functions scaled by the state vector. The dashed line depicts the field and the solid lines shows the Gaussian basis functions.} 
...
The spatial frequency of the observed field is shown in Figure~\ref{fig:ObservationFreqAnal}(C). The cut-off frequency is $\nu_{cy} \approx 0.2$~Hz ($-3$~dB point). The observation cut-off frequency is lower than the field cut-off due to the low-pass filtering effect of the sensors. The spatial frequency response of the sensors can be seen in Figure~\ref{fig:ObservationFreqAnal}(A). Using an oversampling parameter of $\rho_{\phi}=1$, the minimum distance between the field basis functions was calculated to be $2.5$~mm to satisfy Shannon's sampling criterion (from equation~\ref{eq:BasisFunctionSeparation}). The distance between basis functions was set to $\Delta_{\phi}=2.5$~mm, which is sufficient to capture the significant dynamics according to our criteria.
Given the basis function separation, the basis function width parameter was set to $\sigma_{\phi}^2=2.5$~mm. This allowed a smooth representation of the field by the basis function decomposition up to a spatial frequency $\mathbf{\nu_{c\phi}} \approx 0.12$~Hz, as described by equation~\ref{eq:CutoffFromBasisFuncWidth}. The band-limiting effect is due to the spatial frequency response of the basis functions as seen in Figure~\ref{fig:ObservationFreqAnal}(B). The spatial frequency of the reconstructed field from the basis function decomposition is shown in Figure~\ref{fig:FFTTrueEstimate}(B). This figure illustrates the band-limiting effect of the basis function decomposition.
\begin{figure*}[!ht] 
	\begin{center}
...
\mike{The previous paragraph seems to say this: we get good estimates, but with wide variance. We could shrink those aspects of the variance to do with observation noise by including an input. Then, in a practical setting, do multiple runs to get a distribution of parameters. It seems to me there are two messages here. 1: we could reduce the variance using an input. 2: we could deal with the fact that we get a wide range of parameter estimates in practice by conducting multiple trials. I think these messages could be made a bit more clearly, and turned on their heads a bit. I've had a go at this paragraph again.}

\mike{This paper has presented a method for estimating intracortical connectivity and synaptic dynamics from electrophysiological data. As seen in  Section~\ref{sec:state_and_param_results}, the results indicate ubiased estimation, where the mean parameter estimates are in good agreement with the actual values. The estimates of the connectivity kernel parameters show relatively broad distributions, which can be expected considering the system's input is modelled as a random disturbance, and the observations are corrupted by noise.}

\mike{To overcome the effect of noise, an evoked-response paradigm should be considered when applying the estimation algorithm to electrophysiological recordings. Given knowledge of the input, it is expected that the variance of the parameter estimates will decrease.}

\mike{Even with an evoked potential, our results imply that we should only expect to learn distributions of parameters, rather than hope to uncover a `true' parameter set. By designing an input signal carefully, data could be epoched into a sufficient number of trials (realisations), where each trial is a response to a stimulus, such that a distribution of the parameter estimates can be formed.}
...
This paper builds on \cite{schiff2008kalman}, where the estimator is constructed to approximate a continuous field and the dimensionality of the state vector is independent of the resolution of the observation process. Instead, the dimensionality is governed by the expected complexity of the underlying field, which we measure using the frequency analysis methods presented in Section~\ref{SpectralAnalysisSection}. These methods provide a rigorous framework for determining the dimensionality of the system using inherent spatial properties of the field. This enables estimation of a larger field, with a lower dimensional state vector than would be possible otherwise. The basis function decomposition also enabled the coupling to be described by an inner product rather than a spatial convolution. This transformation greatly simplifies the computational complexity of the estimator, which is important for the estimator to run in a real-time scenario.

\mike{The last part of this paragraph seems to be unrelated to the first part. So it reads like: hey we totally pick the dimension of our model based on the data, and we do it carefully. Oh and we can do some funky manipulation deep in the middle of our derivation that lets us speed up life immensely. Maybe the bit about the convolution should come at some other point?}

\mike{In addition, I think the argument associated with this inner product thing should go as follows. `The fact that the connectivity kernel used is isotropic about the origin means that we are able to re-write the spatial mixing effect of the kernel on the firing rate as a convolution of the kernel on the membrane voltage field basis functions. This simplifies.....' Do we talk about this in the body of the paper because it's a really subtle point.}

The frequency analysis and model selection tools presented in Section~\ref{SpectralAnalysisSection} specify the sensor arrangement that is required for capturing the dominant cortical dynamics. Importantly, these new tools can also be used to design electrophysiological recording systems to avoid spatial aliasing. This analysis is becoming increasingly relevant as recording systems are becoming more sophisticated with higher density recordings~\cite{Brinkmann2009}, \mike{ and highlights the relationship between the spatial width of the sensors and the noise in the data}.
...
\mike{again, this paragraph seems to have multiple messages. At the top is hey there are other techniques, and our is better than them. Then there's a repeat of the ``our state dimension is indie of observations'' argument which we already made above, then there's something about multi-resolution models and how two frameworks could be combined to give anisotropy. I quite like the bit about other models, but do we really need to speculate about multi-res and anisotropy? I don't think the relationship is particularly clear...}
...
\begin{tabular}{|l|l|l|}
	...
	\hline
   	...
   	$\Gamma$ & inner product of field basis functions & n.a.\\
	...
	$\nu$, $\nu_c$ & spatial frequency and spatial cut-off frequency & Hz \\
	...
	\hline
\end{tabular}
\begin{flushleft}The first column of the table gives the symbols for all the notation used the paper. The second column provides a brief description of the quantity, and the third column provide the relevant SI units.
\end{flushleft}
\label{tab:label}
\end{table}
