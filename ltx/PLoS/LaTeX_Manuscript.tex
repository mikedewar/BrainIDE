...
The fact that the connectivity kernel is isotropic about the origin means that we are able to re-write the spatial mixing effect of the kernel on the firing rate as a convolution of the kernel on the membrane voltage field basis functions. This transformation greatly simplifies the computational complexity of the estimator, which is important for the estimator to run in a real-time scenario.
