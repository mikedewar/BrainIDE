...
\begin{align}\label{eq:term3}
	...
	&= \varsigma w \ast \mathbf{E}[R_{y_ty_{t+1}}(\boldsymbol\tau)]  + \mathbf{E}[c_t].
\end{align}
...
\begin{align}\label{eq:Auto&CrossNoisy}
	...
	&+ T_s \varsigma w \ast \mathbf{E}[R_{y_ty_{t+1}}(\boldsymbol\tau)]  + \mathbf{E}[c_t] \nonumber \\
	&+\mathbf{E}[((m\ast e_t)\star y_{t+1})(\boldsymbol\tau)] \nonumber \\ 
	&+\sigma_{\epsilon}^2\delta(\boldsymbol{\tau}).
\end{align}
The second term in equation~\ref{eq:Auto&CrossNoisy} can also be simplified to 
\section{Results}
