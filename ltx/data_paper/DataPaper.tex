...
\end{equation}   
where $h(\cdot)$ is the synaptic response kernel and $g(\cdot)$ describes the mean firing rate. The firing rate is described by spatial convolution
\begin{equation}
...
\end{equation}
where $f(\cdot)$ relates mean post-synaptic potentials to mean firing rates, $w(\cdot)$ is the spatial connectivity kernel and $\Omega$ is the spatial domain representing the cortical sheet or surface. The firing rate function is the sigmoidal activation function described by
\begin{equation}
...
\begin{equation}
	...
	\textrm{d}\mathbf{r}'dt'.
\end{equation}
To arrive at the general intergral-differential equation form of the model, we shall express the synaptic response kernel as a Green's function 
\begin{equation}
