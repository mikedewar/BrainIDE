...
The algorithm for the modified URTSS can be seen in Figure~\ref{tab:URTSSAlgorithm}.
...
where 
\begin{align}
	\beta &= \sum_{t=1}^T\mathbf E_{(\theta',\xi')}\left[ \ln \alpha-\mathbf x_{t+1}^\top\boldsymbol\Sigma_w^{-1}\mathbf x_{t+1}\right] \\
...
\end{align}
is constant with respect to $\boldsymbol \theta$ and will disappear by differentiation, and
