...
Figure~\ref{fig:SpatialCrossCorrelation} shows the average (across time) cross-correlations between consecutive field observations for the different time periods relative to the seizure progression. By averaging over time, the effect of the disturbance is reduced. The subplots of the figure show that the connectivity kernel exhibits local excitation with surround inhibition. The plot indicate the support for excitatory aspect of the kernel centred at the origin is -2 mm to 2 mm in the x direction and slightly wider in the y direction. 

The assumption of homgeneity can be tested by performing the correlation analysis 
\begin{itemize}
...
&=\mathbf E_{\boldsymbol (\theta',\xi')}\left[\sum_{t=1}^{T}\ln p(\mathbf y_t|\mathbf x_t)+\sum_{t=1}^{T}\ln p(\mathbf x_t|\mathbf x_{t-1};\boldsymbol \theta, \xi)+\sum_{t=1}^{T}\ln p(\mathbf x_0)\right],
\end{align}
...
\Xi_1&=\mathbf E_{(\theta',\xi')}\left[\sum_{t=1}^T\mathbf x_t\mathbf x_{t+1}^\top\right]=\sum_{t=1}^T\left(\mathbf M_t+\mathbf{\hat x}_t\mathbf{\hat x}_{t+1}^\top\right) \\
\Xi_2&=\sum_{t=1}^T\left[q^\top(\mathbf{\hat x}_t)\boldsymbol\Sigma_e^{-1}q(\mathbf{\hat x}_t)+\Lambda_t(\mathbf r)\mathbf P_t \tilde{\Lambda}_t(\mathbf r)\right] \\
 ...
 \Xi_4&=\mathbf E_{(\theta',\xi')}\left[\sum_{t=1}^T\mathbf x_t\mathbf x_{t}^\top\right]=\sum_{t=1}^T\left(\mathbf P_t+\mathbf{\hat x}_t\mathbf{\hat x}_t^\top\right)
\end{align}
and
\begin{align}
