...
\section{Discussion}
The correlation analysis specifies approximate supports for both disturbance covariance function and connectivity kernel. Since the analysis assumes the sigmoidal firing rate to be in the active region, the amplitudes for the kernel and the disturbance cannot be inferred this way. Although the unknown parameters $\xi$ and $\sigma_{\varepsilon}^2$ can be chosen as described in Correlation analysis section, their values influence the exact shape of the kernel significantly, this step might be considered as an initialisation for the EM algorithm. Knowing the spatial extent of the kernel greatly increases the speed of the algorithm as the number of basis functions to decompose the kernel reduces significanly compared to the case in which the basis funtions are laid out across the entire spatial domain. This also improves the uncertainty of the parameter estimates as the number of parameters to be estimated reduces. On the other hand, the approximated support of the disturbance covariance function facilitates the implementation of the EM algorithm as the disturbance covariance matrix can be factorised into a constant matrix which only depends on the inferred support and an unknown scaling parameter which needs to be estimated.
