...
Having approximated $q\left(\mathbf x_t\right)$, by substituting  \eqref{eq:qTaylor} into equations (\ref{eq:Xi3}-\ref{eq:Xi5}) an approximate to the non-linear $\boldsymbol\Xi$-variables can be calculated which in turn provides an approximate lower bound on the log-likelihood function (see supporting information), 
\begin{align}
...
Given the state estimates, $\mathbf {\hat x}_t$ the state covariance $\mathbf P_t$ and cross-covariance $\mathbf M_t$ , the $\boldsymbol\Xi$-variables can be computed directly.  The computational complexity of the algorithm is given in Table~\ref{table:ComputationalComplexity}. In this table only the computational complexity of variables specific to the presented algorithm are shown, and the computational complexity of the URTSS can be found in \cite{Sarkka2010}.
...
\end{center}
\caption{{\bf Connectivity kernel support estimate.} The estimated support of the kernel is shown by the red circle. Black area shows the non-zero values of the support after thresholding. A $5\times5$ grid of basis functions (blue circles) used to reconstruct the connectivity kernel.}
