...
Although the explanatory power of the theoretical models is rather vast, a limitation has been establishing a rigorous method for choosing model parameters. For example, over the last decade it has come to light that there is a significant variability in parameters of neural models across subjects (ref Marder 2006). For example, it has been shown that a target network behaviour may be achieved by continuum of parameter combinations (Tail wagging the ref). We expect an analogous situation in disease states and pathological activity such as epileptic seizures. Therefore, for models to be clinically useful they must be subject-specific. 

To further motivate the development of a framework for patient-specific neural models we will use intracranial EEG monitoring for planning epilepsy surgery as an example. During iEEG monitoring high fidelity spatiotemporal recordings of the electrical fields of the brain are undertaken. The goal is of the clinical work is to record the electrical activity of seizures and locate their origin. This information along with other behaviour and psychophyics studies enables the definition of surgical margins for the resection of pathological tissue. The ability to fuse the iEEG with a model will not only provide 
