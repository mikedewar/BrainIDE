...
for n=1:L 
    for m=1:length(theta)
        ...
        % theta and xi
        psi_phi = psi_phi_coefficient(m)*...
            Define2DGaussian(mu_phi(1,n), mu_phi(2,n), sigma_psi(m)^2+sigma_phi^2, 0,NPoints,SpaceMin,SpaceMax);

        psi_phi_basis(m,n,:) = psi_phi(:);  
        theta_psi_phi_basis(m,n,:) = theta(m)*psi_phi_basis(m,n,:);
