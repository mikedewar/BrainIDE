...
class IDE():
	...
	def __init__(self,kernel,field_space,EEG,act_fun,alpha,field_noise_variance,beta_variance,obs_noise_covariance,spacestep,init_field,initial_field_covariance,Ts):
	...
	def simulate(self,T):
		...
		Y=[]
		V_vector=[] #I don't save initial field
		V_matrix=[]
		v_membrane=self.init_field
		spiking_rate=pb.array([self.act_fun(float(v_membrane[i,0])) for i in range(v_membrane.shape[0])])
		spatial_location_num=(len(self.field_space))**2

		for t in T[1:]:

			v = self.Svc*pb.matrix(np.random.randn(len(self.EEG.centers),1))
			w = self.Swc*pb.matrix(np.random.randn(spatial_location_num,1))
			print "simulation at time",t

			Kernel_convolution_at_s=signal.convolve2d(self.K,pb.vstack(pb.split(spiking_rate,len(self.field_space))),mode='same',boundary='wrap')
			...
			Y.append((self.spacestep**2)*(self.EEG.ob_kernels_values*v_membrane)+v)
		return V_vector,V_matrix,Y
...
def avi_field(V_matrix,f_space,spacestep,filename,play=0):
	...
	for t in range(len(V_matrix)):
		...
		#pb.axis([f_space[spacestep+1],f_space[-spacestep-1],f_space[spacestep+1],f_space[-spacestep-1]])
		pb.colorbar()
...
def obs_frequency_response(Y,spacestep,db=0,save=0,filename='filename'):
	...
	for i in range(len(Y_matrix)):
		y=y+pb.absolute(pb.fft2(Y_matrix[i]))

	if db:	y=10*pb.log10(y/len(Y_matrix))
	else:	y=y/len(Y_matrix)
	...
	pb.imshow(y,origin='lower',extent=[0,freq_range,0,freq_range],interpolation='nearest',cmap=pb.cm.gray,vmin=y.min(),vmax=y.max())
	pb.colorbar(shrink=.55)
...
def kernel_frequency_response(K,spacestep,db=0,save=0,filename='filename'):
	...
	if db:	k_f=pb.log10(pb.absolute(pb.fft2(K)))
	else:	k_f=pb.absolute(pb.fft2(K))
